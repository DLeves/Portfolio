...
Nem fut le a VIF függvény, mégpedig azért, mert tökéletes multikollineraitás van a modellben. Ez megmagyarázza a 22. kerület NA értékét is.
Megpróbálom a kerületek nélkül is megnézni a VIF mutatót.


```{r}
vif(lm(log(KinArMFt) ~ Terulet + Terasz + Szoba + Felszoba + Furdoszoba + Emelet + DeliTaj + Buda,df))
```
Így már lefut a VIF, azonban van a terület és a szobák számánál így is zavaró mutlikollinearitás van.
...
Mind az AIC, mind a BIC jobban preferálja, ha a kerületek szerepelnek a magyarázó változók között. 
